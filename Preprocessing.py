...
def combine_by_timerange_pandas(dname='r4.2'):
    ...
    http_path = os.path.join(os.getcwd(), 'http.csv')
    ...
    for act in allacts:
        act_handles[act] = open(act + '.csv', 'r', encoding='utf-8')  # Open with UTF-8 encoding
    ...
    while sum(stop.values()) < 5:
        ...
        for act in allacts:
            if 'email' == act:
                ...
                elif dname in ['r6.1', 'r6.2', 'r5.2', 'r5.1']:
...
def get_mal_userdata(data='r4.2', usersdf=None):
    ...
    if not os.path.isdir('answers'):
        ...
        os.system('powershell -Command "(New-Object Net.WebClient).DownloadFile(\'https://kilthub.cmu.edu/ndownloader/files/24857828\', \'answers.tar.bz2\')"')
    ...
    # Special case for 'r6.2' dataset
    if data == 'r6.2': 
        listmaluser.at[listmaluser['scenario'] == 4, 'start'] = '02' + listmaluser[listmaluser['scenario'] == 4]['start']
    ...
    if type(usersdf) != pd.core.frame.DataFrame:
        usersdf = getuserlist(data)  # Assuming getuserlist is a function defined elsewhere
    
    # Initialize columns in usersdf
    usersdf['malscene'] = 0
    usersdf['mstart'] = None
    usersdf['mend'] = None
    usersdf['malacts'] = None
    
    # Loop through listmaluser
    for i in listmaluser.index:
        # Update usersdf with malicious user information
        ...
        # Read malicious user activities
        if data in ['r4.2', 'r5.2']:
            malacts = open(f"answers/r{listmaluser['dataset'][i]}-{listmaluser['scenario'][i]}/" + listmaluser['details'][i], 'r').read().strip().split("\n")
...
if __name__ == "__main__":
    ...
    if dname not in ['r4.1','r4.2','r6.2','r6.1','r5.1','r5.2']:
        raise Exception(f'Please put this script in and run it from a CERT data folder (e.g. r4.2). Current directory: {dname}')
    #make temporary folders
    [os.mkdir(x) for x in ["tmp", "ExtractedData", "DataByWeek", "NumDataByWeek"] if not os.path.exists(x)]
    ...
    # lastProcessedWeek = 62
    # Parallel(n_jobs=numCores)(delayed(process_week_num)(i, users, data=dname) for i in range(lastProcessedWeek,numWeek))
