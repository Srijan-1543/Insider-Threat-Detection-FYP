...
def create_edge_index(X):
    columns = ['role', 'f_unit', 'dept', 'team']  # Specify columns for creating edges
    node_indices = {}   # Create a dictionary to store node indices for each unique combination of values
    
    for col in columns:
        unique_values = np.unique(X[:, columns.index(col)])
        for value in unique_values:
            # Filter array to get indices of rows with the same value in the column
            indices = np.where(X[:, columns.index(col)] == value)[0]
            if value not in node_indices:
                node_indices[value] = indices
            else:
                node_indices[value] = np.concatenate([node_indices[value], indices])

    # Create edges between nodes with the same values in specified columns
    edges = []
    for col in columns:
        for value, indices in node_indices.items():
            if len(indices) > 1:  # Only create edges if there are at least two nodes with the same value
                # Generate combinations of node indices
                pairs = list(combinations(indices, 2))
                edges.extend(pairs)

    edges = list(set(edges))    # Remove duplicate edges
    edge_index = np.array(edges).T.copy()     # Convert edges to edge_index format

    return edge_index


edge_index_train = create_edge_index(X_train)
edge_index_val = create_edge_index(X_val)
...
edge_index_train = torch.LongTensor(edge_index_train)
edge_index_val = torch.LongTensor(edge_index_val)
edge_index_test = torch.LongTensor(edge_index_test)
